...
function buildSystemPrompt() {
  return [
    ...
    "",
    "HEALTH DEPTH (health domain only): if flags.healthAreas / flags.healthNotes are present, name the STRESSED AREAS clearly and give them STRONG, NOTICEABLE emphasis (this helps the reader take extra precautions and seek professional care early) — but NEVER name a specific disease, diagnosis, or predict a specific medical outcome, and never do so for a child. Frame as: 'This chart asks for particular attention to [area] — worth proactive check-ups and professional guidance.' Pair each stressed area with the closest MD/AD window from dashaTiming when it is most active, so the reader knows WHEN to be extra vigilant. Keep it precaution-oriented and empowering, and rely on the app's medical disclaimer.",
    "",
